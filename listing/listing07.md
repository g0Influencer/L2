...
он закрыт,а значением по умолчанию для закрытого целочисленного канала является 0. 
Чтобы избежать данной ошибки, нам нужно проверить в каждом case закрыт ли канал,из которого мы получаем значение.
```
